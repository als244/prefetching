...
void init_arrays(DATA_TYPE* data, DATA_TYPE* data_gpu, unsigned int M, unsigned int N)
{
	int i, j;
...
void correlation(DATA_TYPE* data, DATA_TYPE* mean, DATA_TYPE* stddev, DATA_TYPE* symmat, unsigned int M, unsigned int N)
{
...
void compareResults(DATA_TYPE* symmat, DATA_TYPE* symmat_outputFromGpu, unsigned int M, unsigned int N)
{
...
__global__ void mean_kernel(DATA_TYPE *mean, DATA_TYPE *data, unsigned int M, unsigned int N)
{
...
__global__ void std_kernel(DATA_TYPE *mean, DATA_TYPE *std, DATA_TYPE *data, unsigned int M, unsigned int N)
{
	unsigned int j = blockIdx.x * blockDim.x + threadIdx.x + 1;
...
__global__ void reduce_kernel(DATA_TYPE *mean, DATA_TYPE *std, DATA_TYPE *data, unsigned int M, unsigned int N)
{
	unsigned int j = blockIdx.x * blockDim.x + threadIdx.x + 1;
...
__global__ void corr_kernel(DATA_TYPE *symmat, DATA_TYPE *data, unsigned int M, unsigned int N)
{
	unsigned int j1 = blockIdx.x * blockDim.x + threadIdx.x + 1;
...
void correlationCuda(DATA_TYPE* data_gpu, DATA_TYPE* mean_gpu, DATA_TYPE* stddev_gpu, DATA_TYPE* symmat_gpu, unsigned int M, unsigned int N)
{
...
int main(int argc, char** argv)
{
	...
	init_arrays(data, data_gpu, M, N);
    
	GPU_argv_init();

	correlationCuda(data_gpu, mean_gpu, stddev_gpu, symmat_gpu, M, N);

	t_start = rtclock();
	correlation(data, mean, stddev, symmat, M, N);
	t_end = rtclock();

	fprintf(stdout, "CPU Runtime: %0.6lfs\n", t_end - t_start);
    
	compareResults(symmat, symmat_gpu, M, N);
