...
void init_array(DATA_TYPE* A, DATA_TYPE* x1, DATA_TYPE* x2, DATA_TYPE* y1, DATA_TYPE* y2,
		DATA_TYPE* a_gpu, DATA_TYPE* x1_gpu, DATA_TYPE* x2_gpu, DATA_TYPE* y_1_gpu, DATA_TYPE* y_2_gpu, unsigned int N)
{
	unsigned int i, j;
...
void runMvt(DATA_TYPE* a, DATA_TYPE* x1, DATA_TYPE* x2, DATA_TYPE* y1, DATA_TYPE* y2, unsigned int N)
{
	unsigned int i, j;
...
void compareResults(DATA_TYPE* x1, DATA_TYPE* x1_outputFromGpu, DATA_TYPE* x2, DATA_TYPE* x2_outputFromGpu, unsigned int N)
{
	unsigned int i, fail;
	fail = 0;
...
__global__ void mvt_kernel1(DATA_TYPE *a, DATA_TYPE *x1, DATA_TYPE *y_1, unsigned int N)
{
...
__global__ void mvt_kernel2(DATA_TYPE *a, DATA_TYPE *x2, DATA_TYPE *y_2, unsigned int N)
{
	unsigned int i = blockIdx.x * blockDim.x + threadIdx.x;
...
void mvtCuda(DATA_TYPE* a_gpu, DATA_TYPE* x1_gpu, DATA_TYPE* x2_gpu, DATA_TYPE* y_1_gpu, DATA_TYPE* y_2_gpu, unsigned int N)
{
...
int main(int argc, char** argv)
{
	...
	init_array(a, x1, x2, y_1, y_2, a_gpu, x1_gpu, x2_gpu, y_1_gpu, y_2_gpu, N);
	
	GPU_argv_init();

	mvtCuda(a_gpu, x1_gpu, x2_gpu, y_1_gpu, y_2_gpu, N);
	...
	compareResults(x1, x1_gpu, x2, x2_gpu, N);
